...
//        Token token = CommonUtil.getToken(WeChatConfig.APP_ID,WeChatConfig.APP_SECRET);//获取接口访问凭证access_token
        ...
        List<MaterialSourceDao> lists = CommonUtil.getMaterial(accessToken, "news", 0, 10);//调用获取素材列表的方法
        System.out.println(lists.size());//输出
    }

    @Test
    public void testGetToken2() {
        AccessTokenResult token = CommonUtil.getToken(WeChatConfig.APP_ID_GZ, WeChatConfig.APP_SECRET_GZ);//其中的WeChatConfig方法中有定义的自己微信公众号的appid和appsecret值 用到时直接调用
